...
import type {
  ...
  UpdateOrganizationNotificationsDto,
...
class OrganizationApi extends BasicClientApi {
  ...
  async patch(id: string, body: Partial<UpdateOrganizationDto>): Promise<ApiOrganization> {
    return (
      await this.http.patch<ApiOrganization>(
        `${this.basePath}/${id}`,
        toBackendOrgUpdate(body as UpdateOrganizationDto),
      )
    ).data
  }
  ...
  async uploadLogo(id: string, file: File): Promise<{ avatarUrl: string }> {
    ...
    return (
      await this.http.post<{ avatarUrl: string }>(`${this.basePath}/${id}/logo`, form, {
  ...
  async uploadCover(id: string, file: File): Promise<{ coverUrl: string }> {
    ...
    return (
      await this.http.post<{ coverUrl: string }>(`${this.basePath}/${id}/cover`, form, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
    ).data
  }
  ...
  async updateMyProfile(body: UpdateOrganizationDto): Promise<ApiOrganization> {
    return (await this.http.post<ApiOrganization>('/auth/organizations/settings/profile', toBackendOrgUpdate(body)))
      .data
  }
  ...
  async changeMyPassword(body: ChangeOrganizationPasswordDto): Promise<{ message: string }> {
    return (
      await this.http.post<{ message: string }>('/auth/organizations/settings/password', {
  ...
  async updateMySecurity(body: UpdateOrganizationSecurityDto): Promise<ApiOrganization> {
    return (
      await this.http.post<ApiOrganization>('/auth/organizations/settings/security', {
        two_factor_enabled: body.twoFactorEnabled,
      })
    ).data
  }

  async updateMyNotifications(body: UpdateOrganizationNotificationsDto): Promise<ApiOrganization> {
    return (
      await this.http.post<ApiOrganization>('/auth/organizations/settings/notifications', {
