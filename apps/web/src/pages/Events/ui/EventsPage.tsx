  ...
  const filterCities = useMemo(
    () =>
      [
        ...new Set(
          allEvents.flatMap(
            (event) => [event.locationFrom, event.locationTo, event.location].filter(Boolean) as string[],
          ),
        ),
      ].sort(),
    [allEvents],
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <EventsFilterBar
        ...
        format={f.format}
        onFormatChange={f.setFormat}
        selectedTags={f.selectedTags}
        onTagsChange={f.setSelectedTags}
        dateRange={f.dateRange}
        onDateRangeChange={f.setDateRange}
        location={f.location}
        onLocationChange={f.setLocation}
