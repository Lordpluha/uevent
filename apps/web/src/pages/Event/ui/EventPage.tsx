...
export function EventPage() {
  ...
  const eventSubMutation = useMutation({
    mutationFn: (subscribe: boolean) => (subscribe ? eventsApi.subscribe(id ?? '') : eventsApi.unsubscribe(id ?? '')),
  ...
  const orgFollowMutation = useMutation({
    ...
    onSuccess: async (_data, follow) => {
      await queryClient.invalidateQueries({ queryKey: ['org-subscription', orgId] })
      ...
      toast.success(follow ? t.organizations.subscribed : t.organizations.unsubscribed)
    },
    onError: () => toast.error(t.errors.somethingWrong),
  })
          ...
          <div className="flex flex-wrap gap-2">
            {event.attendees.map((a) => (
              <Link
                key={a.id}
                to={`/users/${a.id}`}
                className="flex items-center gap-1.5 rounded-full border border-border/60 bg-card px-3 py-1 text-xs font-medium transition-colors hover:bg-accent"
              >
      ...
      <section className="mb-8">
        <PromoCodeSection
          ...
          onApplyPromo={(promo) => {
            ...
            toast.success(
              t.events.promoApplied
                .replace('{{code}}', promo.code)
                .replace('{{discount}}', String(promo.discountPercent)),
            )
          }}
          onRemovePromo={() => {
