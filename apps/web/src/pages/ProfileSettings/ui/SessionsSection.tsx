...
import { formatSessionDate, parseUserAgent } from './sessionHelpers'
...
export function SessionsSection() {
  ...
  const revokeSessionMutation = useMutation({
    ...
    onError: () => {
      toast.error(t.profileSettings.sessions.revokeFailed)
      setRevokeSessionCode('')
    },
  })
  ...
  return (
    <>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t.profileSettings.sessions.confirmDesc}</AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (revokeSessionId)
                  revokeSessionMutation.mutate({ sessionId: revokeSessionId, code: revokeSessionCode })
              }}
              disabled={revokeSessionMutation.isPending || revokeSessionCode.length !== 6}
            >
              {revokeSessionMutation.isPending
                ? t.profileSettings.sessions.revoking
                : t.profileSettings.sessions.revokeSession}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog
        open={!!confirmRevokeCurrentId}
        onOpenChange={(o) => {
          if (!o) setConfirmRevokeCurrentId(null)
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t.profileSettings.sessions.revokeCurrentTitle}</AlertDialogTitle>
            <AlertDialogDescription>{t.profileSettings.sessions.revokeCurrentDesc}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={revokeSessionMutation.isPending}>{t.common.cancel}</AlertDialogCancel>
            <AlertDialogAction
              ...
              onClick={() => {
                if (confirmRevokeCurrentId) revokeSessionMutation.mutate({ sessionId: confirmRevokeCurrentId })
              }}
              disabled={revokeSessionMutation.isPending}
            >
              {revokeSessionMutation.isPending
                ? t.profileSettings.sessions.revoking
                : t.profileSettings.sessions.logoutRevoke}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

      <p className="mb-4 text-sm text-muted-foreground">{t.profileSettings.sessions.description}</p>
      ...
      <div className="space-y-3">
        {sessions.map((session: UserSessionInfo) => {
          ...
          return (
            <div
              key={session.id}
              className={`rounded-xl border p-5 ${session.is_current ? 'border-primary/40 bg-primary/5' : 'border-border/60 bg-card'}`}
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <DeviceIcon
                    className={`mt-0.5 h-5 w-5 shrink-0 ${session.is_current ? 'text-primary' : 'text-muted-foreground'}`}
                  />
                  <div className="space-y-1">
                    <p className="flex items-center gap-2 text-sm font-medium">
                      {t.profileSettings.sessions.browserOn
                        .replace('{{browser}}', parsed.browser)
                        .replace('{{os}}', parsed.os)}
                      {session.is_current && (
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t.profileSettings.sessions.lastActive}{' '}
                      {formatSessionDate(session.last_active_at, t.profileSettings.sessions)}
                      {' · '}
                      {t.profileSettings.sessions.created}{' '}
                      {formatSessionDate(session.created_at, t.profileSettings.sessions)}
                    </p>
                  </div>
                </div>
                <Button
