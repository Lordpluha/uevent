      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field>
            ...
            <Input id="org-name" value={profileForm.title} onChange={(e) => setProfileForm((prev) => ({ ...prev, title: e.target.value }))} placeholder={t.orgAccount.profile.name} />
          </Field>
          <Field>
            <FieldLabel htmlFor="org-slogan">{t.orgAccount.profile.slogan}</FieldLabel>
            <Input id="org-slogan" value={profileForm.slogan} onChange={(e) => setProfileForm((prev) => ({ ...prev, slogan: e.target.value }))} placeholder={t.orgAccount.profile.sloganPlaceholder} />
          </Field>
        </div>

        <Field>
            <FieldLabel htmlFor="org-description">{t.common.description}</FieldLabel>
          <Textarea id="org-description" value={profileForm.description} onChange={(e) => setProfileForm((prev) => ({ ...prev, description: e.target.value }))} className="min-h-24" placeholder={t.orgAccount.profile.descriptionPlaceholder} />
        </Field>

        <div className="grid gap-4 sm:grid-cols-3">
          <Field>
            <FieldLabel htmlFor="org-category">{t.common.category}</FieldLabel>
            <Input id="org-category" value={profileForm.category} onChange={(e) => setProfileForm((prev) => ({ ...prev, category: e.target.value }))} placeholder={t.orgAccount.profile.categoryPlaceholder} />
          </Field>
          <Field>
            <FieldLabel htmlFor="org-location">{t.orgAccount.profile.city}</FieldLabel>
            <Input id="org-location" value={profileForm.location} onChange={(e) => setProfileForm((prev) => ({ ...prev, location: e.target.value }))} placeholder={t.orgAccount.profile.cityPlaceholder} />
          </Field>
          <Field>
            <FieldLabel htmlFor="org-phone">{t.common.phone}</FieldLabel>
            <Input id="org-phone" value={profileForm.phone} onChange={(e) => setProfileForm((prev) => ({ ...prev, phone: e.target.value }))} placeholder={t.orgAccount.profile.phonePlaceholder} />
          </Field>
        </div>
