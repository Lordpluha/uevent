  ...
  return (
    <section className="mt-5 rounded-xl border border-border/60 bg-card p-5">
      ...
      <div className="mt-5 flex items-center gap-3">
        ...
        <Link to="/withdrawal">
          <Button variant="outline" disabled={!isVerified} title={!isVerified ? 'Complete verification first' : undefined}>
            Request withdrawal
          </Button>
        </Link>
      </div>
              ...
              {(wallet?.transactions ?? []).map((tx) => {
                ...
                return (
                  <tr key={tx.id} className="border-t border-border/60">
                    ...
                    <td className={`px-3 py-2 text-right font-medium ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
                      {isPositive ? '+' : '-'}{money(Math.abs(tx.amount), tx.currency)}
                    </td>
                  </tr>
                );
              })}
