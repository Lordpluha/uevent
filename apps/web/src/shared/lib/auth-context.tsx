...
export function clearServerCookies(accountType: AuthAccountType | null): Promise<void> {
  ...
  const endpoint = accountType === 'organization' ? '/api/auth/organizations/logout' : '/api/auth/users/logout'
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  return <AuthContext.Provider value={{ ...state, isReady, setAuthenticated, logout }}>{children}</AuthContext.Provider>
}
